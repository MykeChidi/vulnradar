# vulnscan/_recon/security.py - Security Infrastructure Analysis Module
import asyncio
import ssl
import socket
import aiohttp
...
class SecurityInfrastructureAnalyzer:
    ...
    async def _check_waf_headers(self) -> Dict:
        ...
        try:
            ...
            return {"detected": False, "error": "connection_failed", "message": "Cannot connect to target"}
        except aiohttp.ClientResponseError as e:
                    ...
                    if isinstance(resp, aiohttp.ClientResponse):
                        ...
                        for header in rate_limit_headers:
                            if header in headers:
                                result["detected"] = True
                                result["evidence"].append(f"Found rate limit header: {header}")
    ...
    async def _check_ssl_protocols(self) -> Dict:
        ...
        for protocol_name, protocol_version in protocols_to_test:
            try:
                ...
                def test_connection():
                    with socket.create_connection((self.target.hostname, port), timeout=10) as sock:
    ...
    def _assess_protocol_support(self, results: Dict) -> Dict:
        ...
        if results.get("TLSv1.0", {}).get("supported"):
            ...
            assessment["warnings"].append("TLS 1.0 is deprecated and should be disabled")
            assessment["recommendations"].append("Disable TLS 1.0 support")
        
        if results.get("TLSv1.1", {}).get("supported"):
            assessment["secure"] = False
            assessment["warnings"].append("TLS 1.1 is deprecated and should be disabled")
        ...
        if not results.get("TLSv1.3", {}).get("supported"):
            assessment["recommendations"].append("Consider enabling TLS 1.3 for better security")
                ...
                try:
                    with context.wrap_socket(sock, server_hostname=self.target.hostname) as ssock:
                        ...
                        is_vulnerable = (
                            ...
                            ssock.version() == "TLSv1" and
                            not ("GCM" in cipher_name or "CCM" in cipher_name)
                        )
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.get(self.target.url) as response:
                                ...
                                if "openssl/1.0.1" in server.lower():
                                    version_parts = server.lower().split("openssl/1.0.1")
    ...
    async def _check_xss_protection(self, value: str) -> Dict:
        ...
        try:
            ...
            if mode in ["0", "1"]:
                ...
                if mode == "0":
                    result["recommendations"].append("Consider enabling XSS protection with mode=block")
                    
            else:
                result["issues"].append("Invalid X-XSS-Protection value")
                result["recommendations"].append("Use '1; mode=block' for best protection")
    ...
    async def _check_referrer_policy(self, value: str) -> Dict:
        ...
        if value in valid_values:
            ...
            if value == "unsafe-url":
                result["issues"].append("Using unsafe-url which may leak referrer information")
                result["recommendations"].append("Consider using 'strict-origin-when-cross-origin' instead")
