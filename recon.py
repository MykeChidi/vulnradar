# vulnscan/recon.py - Advanced Reconnaissance module
...
class ReconManager:
    ...
    def __init__(self, url: str, options: Dict = None):
        ...
        if not self.options.get("no_cache", False):
            ...
            if self.options.get("clear_cache", False):
                ...
                for subdir in ["network", "security", "webapp", "infrastructure", "misc"]:
        ...
        self.security_analyzer = SecurityInfrastructureAnalyzer(self.target, self.options)
    ...
    async def run_reconnaissance(self) -> Dict:
        ...
        try:
            ...
            if isinstance(webapp_results, Exception):
                self.logger.error(f"Web application analysis failed: {str(webapp_results)}")
                results["webapp"] = {"error": str(webapp_results)}
            else:
                results["webapp"] = webapp_results
                
            if isinstance(infra_results, Exception):
                self.logger.error(f"Infrastructure mapping failed: {str(infra_results)}")
    ...
    def log_recon_findings(self, recon_results: Dict) -> None:
        ...
        if "network" in recon_results:
            ...
            if "dns" in net_info:
                self.logger.info(f"DNS Analysis: Found {len(net_info['dns'].get('A', []))} IP addresses")
            if "ports" in net_info:
                self.logger.info(f"Port Scan: Found {len(net_info['ports'])} open ports")
                
        if "security" in recon_results:
            sec_info = recon_results["security"]
            if "waf" in sec_info and sec_info["waf"].get("detected"):
                self.logger.info(f"WAF detected: {sec_info['waf'].get('type', 'Unknown')}")
        ...
        if "web_application" in recon_results:
            ...
            if "technologies" in web_info:
                ...
                self.logger.info(f"Detected technologies: {', '.join(t['name'] for t in techs.get('frameworks', []))}")
