# vulnscan/_recon/infrastructure.py - Infrastructure relationship mapping for vulnerability scans.
import asyncio
import ssl
import socket
import dns.resolver
...
class InfrastructureRelationshipMapper:
    ...
    async def _dns_bruteforce(self) -> List[str]:
        ...
        common_subdomains = [
            ...
            "dev", "staging", "api", "cdn", "blog", "shop",
            "app", "m", "mobile", "store", "portal", "beta"
        ]
    ...
    async def _search_engine_discovery(self) -> List[str]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                search_query = f"site:{self.target.hostname}"
                headers = {
                ...
                domain_pattern = rf"[a-zA-Z0-9-]+\.{self.target.hostname}"
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        if data.get("status") == "success":
                            ...
                            self.logger.warning(f"ip-api.com error: {data.get('message')}")
    ...
    async def _identify_third_party_service(self, script_src: str) -> Optional[Dict]:
        ...
        service_signatures = {
            ...
            "marketing": {
                ...
                "Marketo": {
                    "patterns": ["marketo.com", "marketo.net"],
                    "risk": "low"
                }
            },
            "security": {
    ...
    async def _get_ip_block_info(self) -> Dict:
        ...
        try:
            ...
            ip_info["network_ranges"] = [block["network"] for block in ip_info["blocks"]]
