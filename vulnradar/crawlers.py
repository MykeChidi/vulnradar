    ...
    def _initialize_selenium(self):
        ...
        try:
            ...
            error_handler.handle_error(
                ConfigurationError(
                    f"Failed to initialize Selenium: {str(e)}", original_error=e
                ),
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self.driver:
            try:
                ...
                error_handler.handle_error(
                    ResourceError(
                        f"Error closing Selenium: {str(e)}", original_error=e
                    ),
                    context={},
                )
            finally:
            ...
            async with aiohttp.ClientSession(
                ...
                while self.to_visit and self.page_count < self.max_pages:
                    if len(self.visited_urls) > self.max_pages * 1.5:
                        logger.warning(
                            f"URL tracking limit reached: {len(self.visited_urls)}"
                        )
                    ...
                    try:
                        ...
                        error_handler.handle_error(
                            NetworkError(
                                f"Error crawling {url}: {str(e)}", original_error=e
                            ),
                        ...
                        error_handler.handle_error(
                            ScanError(
                                f"Unexpected error crawling {url}: {str(e)}",
                                original_error=e,
                            ),
                            context={"url": url, "depth": depth},
                        )
                ...
                logger.info(
                    f"Crawling finished. Total pages visited: {self.page_count}"
                )
        except Exception as e:
            error_handler.handle_error(
    ...
    def prioritize_endpoints(self, endpoints: List[str]) -> List[str]:
        ...
        def score_endpoint(url: str) -> int:
            ...
            if "?" in url:
                score += 10
            if any(
                x in url_lower for x in ["search", "query", "id=", "user=", "file="]
            ):
                score += 8
            ...
            if any(
                ...
                for ext in [".jpg", ".png", ".css", ".js", ".gif", ".svg"]
            ):
                score -= 5
