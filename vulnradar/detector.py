...
class TechDetector:
    ...
    def __init__(self, timeout: int = 10, max_content_size: int = 5_000_000):
        ...
        self.signatures = {
            # Web servers
            "Apache": [r"Server:\s*Apache", r"Apache/[\d\.]+"],
            "Nginx": [r"Server:\s*nginx", r"nginx/[\d\.]+"],
            ...
            "Joomla": [r"/media/jui/", r"/media/system/js/", r"Joomla!"],
            "Laravel": [r"laravel_session", r"XSRF-TOKEN"],
            "Django": [r"csrfmiddlewaretoken", r"__django", r"\bdjango\b"],
            "Flask": [r"\bWerkzeug\b", r"\bFlask\b"],
            "React": [
                r"react-root",
            ...
            "Vue": [r"vue\.js", r"vue\.min\.js", r"__vue__", r"data-v-[\da-f]+"],
            "Angular": [
                r"ng-app",
                r"ng-controller",
                r"angular\.js",
            ...
            "MySQL": [r"\bMySQL\b", r"mysql_error"],
            "PostgreSQL": [r"\bPostgreSQL\b", r"pg_database"],
            "MongoDB": [r"\bMongoDB\b", r"mongo_err"],
            # Programming Languages
            "PHP": [r"X-Powered-By:\s*PHP", r"\.php\b", r"PHPSESSID"],
            "ASP.NET": [r"\.aspx\b", r"\bASP\.NET\b", r"__VIEWSTATE"],
            "Java": [r"JavaServer Pages", r"\bServlet\b", r"JSESSIONID"],
            "Python": [r"\bPython/[\d\.]+", r"\bwsgi\b", r"__pycache__"],
            "Ruby": [r"\bRuby\b", r"\bRails\b", r"_rails_session"],
            # JavaScript Libraries
            "jQuery": [r"jquery\.js", r"jquery\.min\.js", r"jQuery\s+v[\d\.]+"],
            "Bootstrap": [
                r"bootstrap\.css",
                r"bootstrap\.min\.css",
                r"bootstrap\.js",
    ...
    async def detect(
        ...
        try:
            ...
            error_handler.handle_error(
                ScanTimeoutError(
                    f"Request timeout after {self.timeout.total}s", original_error=e
                ),
            ...
            error_handler.handle_error(
                ScanError(
                    f"Unexpected error: {type(e).__name__}: {str(e)}", original_error=e
                ),
                context={"url": url},
            )
    ...
    def _detect_from_structure(self, soup: BeautifulSoup) -> Dict[str, int]:
        ...
        if soup.find(attrs={"data-reactroot": True}) or soup.find(
            attrs={"data-react-root": True}
        ):
            detected["React"] = max(detected.get("React", 0), 80)

        if soup.find(
            attrs=lambda x: x
            and any(k.startswith("data-v-") for k in x.keys() if isinstance(x, dict))
        ):
