...
from .utils.error_handler import ScanError, get_global_error_handler, handle_errors
...
class ModernButton(tk.Canvas):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            parent, height=40, bg=parent.cget("bg"), highlightthickness=0, **kwargs
        )
        ...
        self.rect = self.create_rectangle(
            0, 0, 200, 40, fill=bg, outline="", tags="btn"
        )
        self.text_id = self.create_text(
            50, 20, text=text, fill=fg, font=("Segoe UI", 10, "bold"), tags="btn"
        )
...
class VulnRadarGUI:
    ...
    def setup_styles(self):
        ...
        style.configure(
            "Card.TFrame", background=self.colors["secondary_bg"], relief="flat"
        )
    ...
    def setup_scan_tab(self):
        ...
        scrollbar = ttk.Scrollbar(
            self.scan_tab, orient="vertical", command=canvas.yview
        )
        ...
        target_card = self.create_card(
            scrollable_frame, "🎯 Target Configuration", 0, 0, columnspan=3
        )
        ...
        progress_card = self.create_card(
            scrollable_frame, "📊 Scan Progress", 2, 0, columnspan=3
        )
    ...
    def setup_recon_tab(self):
        ...
        scrollbar = ttk.Scrollbar(
            self.recon_tab, orient="vertical", command=canvas.yview
        )
        scrollable_frame = tk.Frame(canvas, bg=self.colors["bg"])
        ...
        self.recon_modules = [
            ...
            (
                "Security",
                "recon_security",
                [("SSL/TLS Analysis", "ssl"), ("Security Headers", "sec_headers")],
            ),
            (
                "Miscellaneous",
                "recon_misc",
        ...
        recon_mode_card = self.create_card(
            scrollable_frame, "🔍 Reconnaissance Configuration", 0, 0, columnspan=2
        )
        ...
        tk.Checkbutton(
            recon_mode_inner,
            text="Advanced Recon Only (Skip Vulnerability Scanning)",
        ...
        for i, (category_name, category_key, _) in enumerate(self.recon_modules):
            ...
            self.category_to_options[category_key] = [
                opt[1] for opt in self.recon_modules[i][2]
            ]
        ...
        for i, (category_name, category_key, options) in enumerate(self.recon_modules):
            ...
            card = self.create_card(
                scrollable_frame, f"🔍 {category_name}", row, 0, columnspan=2
            )
    ...
    def setup_logs_tab(self):
        ...
        save_btn = ModernButton(
            btn_frame, "Save Logs", self.save_logs, bg=self.colors["accent"], width=120
        )
    ...
    def update_stats(
        self, endpoints: Optional[int] = None, vulns: Optional[int] = None
    ):
    ...
    def start_scan(self):
        ...
        options = {
            ...
            "recon_infrastructure": self.recon_module_vars[
                "recon_infrastructure"
            ].get(),
            "recon_misc": self.recon_module_vars["recon_misc"].get(),
            "advanced_port_scan": self.recon_vars.get(
                "port_scan", tk.BooleanVar(value=True)
            ).get(),
            "detect_waf": self.recon_vars.get("waf", tk.BooleanVar(value=True)).get(),
            "detect_load_balancers": self.recon_vars.get(
                "load_bal", tk.BooleanVar(value=True)
            ).get(),
            "service_detection": self.recon_vars.get(
                "service_detect", tk.BooleanVar(value=True)
            ).get(),
            "os_detection": self.recon_vars.get(
                "os_detect", tk.BooleanVar(value=True)
            ).get(),
            "script_scan": self.recon_vars.get(
                "script_scan", tk.BooleanVar(value=True)
            ).get(),
            "content_discovery": self.recon_vars.get(
                "content_disc", tk.BooleanVar(value=True)
            ).get(),
            "js_analysis": self.recon_vars.get(
                "js_analysis", tk.BooleanVar(value=True)
            ).get(),
            "dir_enum": self.recon_vars.get(
                "dir_enum", tk.BooleanVar(value=False)
            ).get(),
            "subdomain_enum": self.recon_vars.get(
                "subdomain", tk.BooleanVar(value=True)
            ).get(),
            "cloud_mapping": self.recon_vars.get(
                "cloud", tk.BooleanVar(value=True)
            ).get(),
            "dns_bruteforce": self.recon_vars.get(
                "dns_brute", tk.BooleanVar(value=True)
            ).get(),
            "ssl_analysis": self.recon_vars.get("ssl", tk.BooleanVar(value=True)).get(),
            "security_headers": self.recon_vars.get(
                "sec_headers", tk.BooleanVar(value=True)
            ).get(),
            "error_analysis": self.recon_vars.get(
                "error_analysis", tk.BooleanVar(value=True)
            ).get(),
            "cache_analysis": self.recon_vars.get(
                "cache_analysis", tk.BooleanVar(value=True)
            ).get(),
            "check_debug_mode": self.recon_vars.get(
                "debug_check", tk.BooleanVar(value=True)
            ).get(),
            "check_dev_artifacts": self.recon_vars.get(
                "check_dev_artifacts", tk.BooleanVar(value=True)
            ).get(),
            "backend_tests": self.recon_vars.get(
                "backend_tests", tk.BooleanVar(value=True)
            ).get(),
        ...
        try:
            ...
            if results.get("error"):
                ...
                self.log_message(
                    ...
                    "success" if vulns_count == 0 else "warning",
                )
                ...
                for vuln in results.get("vulnerabilities", []):
                    ...
                    severity_color = (
                        "error" if vuln.get("severity") == "High" else "warning"
                    )
                    self.log_message(
                ...
                self.progress_var.set(
                    f"Scan complete - {vulns_count} vulnerabilities found"
                )
