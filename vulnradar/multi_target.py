...
class MultiTargetScanner:
    ...
    def _load_targets(self):
        ...
        try:
            if file_ext in [".yaml", ".yml"]:
                ...
                raise ValueError(
                    f"Unsupported file format: {file_ext}. Use JSON or YAML."
                )
    ...
    def _add_target(
        ...
        try:
            target = TargetConfig(
                url=url, name=name, options=options, timeout=timeout, retries=retries
            )
    ...
    def _merge_options(
        self, target_options: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def scan_target(
        self, target: TargetConfig, attempt: int = 1
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                results = await asyncio.wait_for(scanner.scan(), timeout=target.timeout)
            ...
            logger.info(
                f"{Fore.GREEN}Completed scan for: {target.name}{Style.RESET_ALL}"
            )
            ...
            logger.error(
                f"{Fore.RED}Scan failed for {target.name} (attempt {attempt}): {str(e)}{Style.RESET_ALL}"
            )
    ...
    async def scan_sequential(self) -> None:
        ...
        with tqdm(
            total=len(self.targets), desc="Scanning targets", unit="target"
        ) as pbar:
    ...
    async def scan_concurrent(self) -> None:
        ...
        logger.info(
            f"{Fore.YELLOW}Running concurrent scans (max {self.max_concurrent})...{Style.RESET_ALL}"
        )
    ...
    def generate_summary(self) -> Dict:
        ...
        for result in self.results:
            ...
            if result.get("scan_completed"):
                ...
                summary["total_vulnerabilities"] = summary[
                ...
                for vuln in vulns:
                    ...
                    if severity in summary["vulnerabilities_by_severity"]:
                        ...
                        summary["vulnerabilities_by_severity"][severity] = (
                            curr_count + 1
                        )

                    vuln_type = vuln.get("type", "Unknown")
                    curr_type_count = summary["vulnerabilities_by_type"].get(
                        vuln_type, 0
                    )
    ...
    def print_summary(self):
        ...
        print(
            f"Successful Scans: {Fore.GREEN}{summary['successful_scans']}{Style.RESET_ALL}"
        )
        ...
        for severity, count in summary["vulnerabilities_by_severity"].items():
            color = (
                Fore.RED
                if severity == "High"
                else Fore.YELLOW if severity == "Medium" else Fore.BLUE
            )
        ...
        if summary["vulnerabilities_by_type"]:
            ...
            for vuln_type, count in sorted(
                summary["vulnerabilities_by_type"].items(),
                key=lambda x: x[1],
                reverse=True,
            ):
