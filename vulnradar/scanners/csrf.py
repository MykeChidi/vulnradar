...
class CSRFScanner(BaseScanner):
    ...
    async def scan(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error scanning '{url}' for CSRF: {str(e)}", original_error=e
                ),
    ...
    async def _test_missing_csrf_token(
        ...
        try:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error testing missing CSRF token: {str(e)}", original_error=e
                ),
                context={"url": url, "scanner": "CSRF"},
            )
    ...
    async def _test_csrf_token_validation(
        ...
        try:
            ...
            vulnerability = await self._submit_csrf_test(
                url, form, test_data, "CSRF token removed"
            )
            if vulnerability:
            ...
            vulnerability = await self._submit_csrf_test(
                url, form, test_data, "Invalid CSRF token"
            )
            if vulnerability:
                return vulnerability
            ...
            vulnerability = await self._submit_csrf_test(
                url, form, test_data, "Empty CSRF token"
            )
            if vulnerability:
                return vulnerability
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error testing CSRF token validation: {str(e)}", original_error=e
                ),
                context={"url": url, "scanner": "CSRF"},
            )
        ...
        try:
            ...
            for form in forms:
                if form["action"] == url or (not form["action"] and url == url):
                    ...
                    try:
                        test_data = (
                            literal_eval(payload) if payload.startswith("{") else {}
                        )
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error validating CSRF vulnerability in '{url}': {str(e)}",
                    original_error=e,
                ),
                context={"url": url, "scanner": "CSRF"},
            )
