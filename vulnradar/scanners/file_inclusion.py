...
class FileInclusionScanner(BaseScanner):
    ...
    async def scan(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error scanning '{url}' for file inclusion: {str(e)}",
                    original_error=e,
                ),
    ...
    async def _test_url_parameters(self, url: str) -> List[Dict]:
        ...
        for param_name, original_value in params.items():
            if any(keyword in param_name.lower() for keyword in self.file_params):
                # Test LFI
                lfi_vulns = await self._test_lfi_parameter(
                    url, param_name, original_value
                )
                vulnerabilities.extend(lfi_vulns)

                # Test RFI
                rfi_vulns = await self._test_rfi_parameter(
                    url, param_name, original_value
                )
    ...
    async def _test_form_inputs(self, url: str) -> List[Dict]:
        ...
        for form in forms:
            for input_field in form["inputs"]:
                if any(
                    keyword in input_field["name"].lower()
                    for keyword in self.file_params
                ):
                    # Test LFI
    ...
    async def _detect_lfi(self, response_text: str, payload: str) -> bool:
        ...
        if "etc/passwd" in payload:
            if re.search(r"root:.*:0:0:", response_text) or re.search(
                r"daemon:", response_text
            ):
                return True

        if "hosts" in payload:
            if re.search(r"127\.0\.0\.1", response_text) or re.search(
                r"localhost", response_text
            ):
                return True
