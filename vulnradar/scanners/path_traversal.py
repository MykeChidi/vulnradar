...
class PathTraversalScanner(BaseScanner):
    ...
    async def scan(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error scanning {url} for path traversal: {str(e)}",
                    original_error=e,
                ),
        ...
        for form in forms:
            for input_field in form["inputs"]:
                if any(
                    ...
                    for keyword in self.vulnerable_params
                ):
    ...
    async def _test_form_field(
        ...
        for payload in self.payloads[
            :20
        ]:  # Test fewer payloads for forms to avoid excessive requests
            try:
                # Create test payload
                test_payloads = [
                    payload + "etc/passwd",
