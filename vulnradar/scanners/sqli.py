    ...
    async def scan(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error scanning '{url}' for SQL injection: {str(e)}",
                    original_error=e,
                ),
        ...
        for param_name, original_value in params.items():
            for payload in self.payloads:
                ...
                try:
                    ...
                    error_handler.handle_error(
                        ScanError(
                            f"Error testing SQL injection on GET parameter {param_name} at {url}: {str(e)}",
                            original_error=e,
                        ),
                        context={
                            "url": url,
        ...
        for input_field in form.get("inputs", []):
            ...
            for payload in self.payloads:
                ...
                try:
                    ...
                    error_handler.handle_error(
                        ScanError(
                            f"Error testing SQL injection on field {field_name} at {action_url}: {str(e)}",
                            original_error=e,
                        ),
                        context={
                            "url": action_url,
    ...
    async def validate(self, url: str, payload: str, evidence: str) -> bool:
        ...
        try:
            ...
            if not params and "=" in url:
                ...
                test_payload = (
                    "' OR '2'='2" if payload != "' OR '2'='2" else "' OR '3'='3"
                )
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error validating SQL injection at '{url}': {str(e)}",
                    original_error=e,
                ),
                context={"url": url, "scanner": "SQLI"},
            )
