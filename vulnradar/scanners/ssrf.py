...
class SSRFScanner(BaseScanner):
    ...
    async def scan(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            for param_name, param_value in url_params.items():
                if any(
                    keyword in param_name.lower() for keyword in self.vulnerable_params
                ):
                    ssrf_findings = await self._test_ssrf_parameter(
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error scanning '{url}' for SSRF: {str(e)}", original_error=e
                ),
    ...
    async def _test_ssrf_parameter(
        ...
        try:
            ...
            for payload in self.payloads:
                ...
                query_string = "&".join(
                    [f"{k}={quote(str(v))}" for k, v in test_params.items()]
                )
                test_url = f"{base_url}?{query_string}"

                # Test the payload
                vulnerability = await self._test_ssrf_payload(
                    test_url, payload, param_name, "parameter"
                )
                if vulnerability:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error testing SSRF parameter {param_name}: {str(e)}",
                    original_error=e,
                ),
    ...
    async def _test_ssrf_form(self, url: str, form: Dict) -> List[Dict]:
        ...
        try:
            ...
            for input_field in form["inputs"]:
                ...
                if not any(
                    keyword in input_name.lower() for keyword in self.vulnerable_params
                ):
                    continue
    ...
    async def _test_ssrf_form_payload(
        self,
        action_url: str,
        method: str,
        form_data: Dict,
