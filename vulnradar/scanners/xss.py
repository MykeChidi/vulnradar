...
class XSSScanner(BaseScanner):
    ...
    async def scan(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error scanning '{url}' for XSS: {str(e)}", original_error=e
                ),
        ...
        for param_name, original_value in params.items():
            for payload in self.payloads:
                ...
                try:
                    ...
                    error_handler.handle_error(
                        ScanError(
                            f"Error testing XSS on GET parameter {param_name} at {url}: {str(e)}",
                            original_error=e,
                        ),
    ...
    async def _check_post_params(self, form: Dict) -> List[Dict]:
        ...
        for input_field in form.get("inputs", []):
            ...
            original_value = input_field.get("value", "")  # noqa
            ...
            for payload in self.payloads:
                ...
                try:
                    ...
                    error_handler.handle_error(
                        ScanError(
                            f"Error testing XSS on {action_url}: {str(e)}",
                            original_error=e,
                        ),
    ...
    def _check_for_xss_reflection(self, response_text: str, payload: str) -> bool:
        ...
        dangerous_contexts = ["script", "style", "svg", "iframe", "object", "embed"]
    ...
    def _extract_reflection_snippet(self, response_text: str, payload: str) -> str:
        ...
        if encoded_index != -1:
            ...
            end_index = min(
                len(response_text), encoded_index + len(encoded_payload) + 50
            )
            snippet = response_text[start_index:end_index]
    ...
    async def _check_dom_xss(self, url: str) -> List[Dict]:
        ...
        for payload in dom_payloads:
            ...
            try:
                ...
                error_handler.handle_error(
                    ScanError(
                        f"Error testing DOM XSS on {url}: {str(e)}", original_error=e
                    ),
        ...
        for script in soup.find_all("script"):
            ...
            for pattern in dangerous_patterns:
                if pattern in script_content:
                    ...
                    end_index = min(
                        len(script_content), pattern_index + len(pattern) + 30
                    )
    ...
    async def validate(self, url: str, payload: str, evidence: str) -> bool:
        ...
        try:
                ...
                if form.get("inputs"):
                    ...
                    for input_field in form.get("inputs", []):
                        if input_field.get("name") == param_name:
                            ...
                            form_data[input_field.get("name")] = input_field.get(
                                "value", ""
                            )
            ...
            error_handler.handle_error(
                ScanError(
                    f"Error validating XSS vulnerability at '{url}': {str(e)}",
                    original_error=e,
                ),
                context={"url": url, "scanner": "XSS"},
            )
