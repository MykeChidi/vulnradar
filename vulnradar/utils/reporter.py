...
class ReportGenerator:
    ...
    def generate_html_report(self, report: Report) -> str:
        ...
        try:
            ...
            error_handler.handle_error(
                ResourceError(
                    f"Failed to write HTML report: {str(e)}", original_error=e
                ),
    ...
    def generate_json_report(self, report: Report) -> str:
        ...
        try:
            ...
            error_handler.handle_error(
                ResourceError(
                    f"Failed to write JSON report: {str(e)}", original_error=e
                ),
                context={"target": report.target, "path": str(path)},
            )
            raise
    ...
    def generate_excel_report(self, report: Report) -> str:
        ...
        if report.is_recon_only:
            ...
            for category, details in report.reconnaissance.items():
                if isinstance(details, dict):
                    ...
                    recon_data.append(
                        {"Category": category, "Key": "-", "Value": str(details)}
                    )
        ...
        try:
            ...
            error_handler.handle_error(
                ResourceError(
                    f"Failed to write Excel report: {str(e)}", original_error=e
                ),
                context={"target": report.target, "path": str(path)},
            )
            raise
